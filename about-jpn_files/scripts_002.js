...
jQuery(document).ready(function() { 
	 ...
	 * Select items
	 */
	jQuery('select:not(.checkout select)').wrap('<div class="select-option2" />').parent();
...
jQuery(document).ready(function() { 
        ...
        jQuery(this).find('.project').each(function() {
            ...
            filterTags.forEach(function(tagName) {
                if (filters.indexOf(tagName) == -1) {
                    filters += '<option data-filter="' + tagName + '">' + capitaliseFirstLetter(tagName) + '</option>';
                }
            });
            
            jQuery(this).closest('.projects').find('select.filters').empty().append('<option data-filter="all" class="active">' + wp_data.all_title + '</option>').append(filters);
            
        });
        console.log(filters);  
    })

    jQuery(".filters").chosen().change(function (event) {
        var filter = jQuery(".filters").val();
        if(filter && filter.length==1){
            jQuery(".filters").closest('.projects').find('.project').addClass('inactive');    
        }    
        jQuery('.projects').find('.project').each(function(item,el) {
            var data_fileter = jQuery(el).data('filter');
            if(filter){
                filter.forEach(function(element) {
                    if (data_fileter==element) {
        ...
        if(filter && filter.length>0){
            filter.forEach(function(element) {
                if ("All"==element) {
                    jQuery(".filters").closest('.projects').find('.project').removeClass('inactive');
                }
              })
        }else if(!filter){
            jQuery(".filters").closest('.projects').find('.project').removeClass('inactive');
    ...
    jQuery('.filters option').click(function() {
        var filter = jQuery(this).attr('data-filter');
        jQuery(this).closest('.filters').find('option').removeClass('active');
        jQuery(this).addClass('active');
